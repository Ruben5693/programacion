...
int main(int argc, char const *argv[]) {
    ...
    int i;
    int N=atoi(argv[1]);
    size_t wi=atoi(argv[2]);
    int *vector_in, *vector_out;
    
    vector_out= (int *)malloc(sizeof(int)*wi);
    ...
    CrearPrograma(program, contexto,num_dispositivos, Dispositivos, "", "programa.cl");
    CrearKernel(kernel,program,"mult_vec");
    //fuincion ocl
    CrearBuffer(contexto, CL_MEM_USE_HOST_PTR, (size_t) N,vector_out, buf_out);
    
    AsignarParametro(kernel, 0, sizeof(cl_mem), &buf_out);
    AsignarParametro(kernel, 1, sizeof(int), &N);
    ...
    int contador=0;
    for(int i=0; i<wi; i++)
        contador=contador+vector_out[i];
    
    float pi=(float)contador*4.0/N;
