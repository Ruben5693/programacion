...
void ocl(int N,double *A,terna_t *ternas, int num_sb, EntornoOCL_t entorno) {
    ...
    CrearBuffer(entorno.contexto, CL_MEM_USE_HOST_PTR, (size_t) N*N,A, buf_in);
    CrearBuffer(entorno.contexto, CL_MEM_USE_HOST_PTR, (size_t) N*N,vector_out, buf_out);
     CrearBuffer(entorno.contexto, CL_MEM_USE_HOST_PTR, (size_t) num_sb,ternas, buf_in2);
    
    AsignarParametro(entorno.kernel, 0, sizeof(int), &N);
    AsignarParametro(entorno.kernel, 1, sizeof(cl_mem), &buf_in);
    AsignarParametro(entorno.kernel, 2, sizeof(terna_t), &buf_in2);
    AsignarParametro(entorno.kernel, 3, sizeof(int), &num_sb);
    AsignarParametro(entorno.kernel, 4, sizeof(cl_mem), &buf_out);
    
    EnviarBuffer(entorno.cola, buf_in, false ,0, N*N*sizeof(double), A,0,NULL, env);
    EnviarBuffer(entorno.cola, buf_in2, false ,0, num_sb*sizeof(terna_t), A,0,NULL, env);
