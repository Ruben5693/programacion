...
int x,y,t,mov_col,Ddiv,Dmod,mov_fil,mf,mc,DdivN,x_col,ini,Ny;


for(int iter=0;iter<num_sb;iter++){
	...
	int ddd[t];
	Ny=N*y;
	for(int datos=0;datos<t*t;datos++){
		Ddiv=datos/t;
		DdivN=Ddiv*N;
		Dmod=datos%t;
		x_col=Dmod+x;
		if(N<=x_col)
			x_col-=N;
		for(int mul=0;mul<t;mul++){

			mov_col=Ny +x_col+mul*N;
		...
		A[ddd[Dmod]]+=A2[mov_fil]*A2[mov_col];
